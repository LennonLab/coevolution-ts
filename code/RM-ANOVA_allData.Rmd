...
In the first few days of the experiment cell numbers dropped below detection limit in 2 of 3 SNO lines. Missing data cannot be analyzed in RM-ANOVA. I will remove these points.

```{r}

# plot total cells of raw data
d%>%
  mutate(cell.ml=if_else(cell.ml>10, cell.ml,NULL))%>%
    ggplot(aes(x=Time, y=cell.ml))+
      ...
      theme(legend.position = "bottom",
            text=element_text(size=14))+
      ylab("cell/ml (log)")+
      coord_cartesian(ylim = c(5e5, 5e9))
```
...
The best model is signofocantly better than the next 1. Looking at its results:
...
**No significant effect of seed.bank on population density at any level.**
