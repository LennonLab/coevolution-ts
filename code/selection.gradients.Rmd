...
data%>%
   ggplot(aes(perc.spore.std)) +
   geom_histogram() +
   facet_wrap(~culture, nrow = 3) +
   xlab("standardized % sporulation") +
   ylab("Frequency") + 
   theme_cowplot()
```
...
d.cor <-
   ...
   select(slope = estimate, seed.bank, phage)%>%
   # add to d.cor
   left_join(d.cor, ., by =c( "seed.bank", "phage")) %>%
   # make label
