...
```{r}
  with(d, boxCox(cell.ml ~ phage*line, 
...
```{r}
# Levene's test with multiple independent variables
car::leveneTest(bxcx.response ~ phage*line, data = d)
```
...
for(Q in c(0:2)){ 
  for (P in c(0:2)){ 
    ...
    #run model
    cur.model <- d%>%
      lme(bxcx.response ~ phage * time.fct , 
          random = ~1|flask, 
...
Ohage and time both significantly effect spore densities, as main effects and in their interaction. Before looking into the potential drivers of these effects we evaluate the model. 
...
```
The line marks the p-values of the selected model, and the points show p-values of all models tested (including best). All the models have the same results. 
...
What in the phage X time interaction is causing a significant effect on population dynamics?  
...
Plot shows that spores are mostly lower in presence of phage.  the phage effect on spore density is consistent across time except for a brief period in the first few samples.
...
coevo.pairs <- pairs(regrid(coevo.emm.bc), simple="phage")%>%
  tidy
...
The major differences seem to be in the first two transfers where there is no seed-bank difference in spore densities  
