...
Though it is not significantly better than the the other models.  
...
Seed-bank treatment has no significant effect on spore population densities, neither as main effect or in interaction with other predictors. phage and time both significantly effect spore densities. Before looking into the potential drivers of this effect we evaluate the model. 

### How sensitive would the result be to model selected?  
```{r}
# put all model results in tibble
all.models <- 
  map(l.rm, anova.lme) %>%
...
The line marks the p-values of the selected model, and the points shoe p-values of all models tested (including best). The results are pretty similar across the models.
...
Spores are consistently lower in presence of phage. a rise in spore density is seen in the second half of the experiment in the absence of phage. This complements the observation of decline on vegetative cells in non-infected WT culture in the same period.

What is the source of the phageXtime interaction being significant? Breaking up the model estimated by these two predictors next.

```{r}

coevo.emm <- emmeans(m.best, ~  phage * time.fct)
coevo.pairs <- pairs(coevo.emm, simple="phage")%>%
  tidy
...
The phage effect on spore density is consistent across time except for a brief period in the first few samples.


The major differences seem to be in phage-treated cultures, between first two transfers and the rest of the data.  

# Summary

Spore densities were significantly lower in phage infected cultures, in agreement with the previous observations of lower vegetative cells and overall cell density. We find no difference in spore densities between the two seed-bank treatments in both long and short seed bank treatments. In the non-infected lines we find a trend on increase in spore densities, mostly in second half of the experiment.