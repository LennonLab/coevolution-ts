...
The results are pretty similar across the models. For the **phageXseed.bank** interaction the effect remains non-significant in all models having a P>0.05 for that interaction. 
...
In the paper I want to describe the overall effect of phage on host population density
...
### Phage effect on population size

```{r}
emm_sb.ph <-
  stats::update(ref_grid(m.best), tran = make.tran("boxcox", param =  bx.cx$roundlam)) %>%
  emmeans(pairwise ~ phage | seed.bank ) 
...
plot(regrid(emm_sb.ph$emmeans))+
  theme_classic()+
  panel_border()+
  scale_x_log10(limits= c(1e7,7e8))+
  annotation_logticks(sides = "b")

  regrid(emm_sb.ph$emmeans) 
