...
It is significantly better than the next models.  
...
As expected time is very significant at all levels. This includes also a significant 3-way interaction phage x seed-bank x time that is detected.  
Phage too is strongly significant as a main effect and in interaction with time. Phage X seed-bank not significant.  
...
The line marks the p-values of the selected model, and the points shoe p-values of all models tested (including best). There is variance regarding  the p-value associated with seed bank X phage treatment. However these remain above significance threshold across, so conclusions would be the same.
...
c. Equal residual variance mostly holds across time. However in the no-phage lines the variance may be increasing with time.    
d. Equal residual variance holds across experimental units (flasks). **Like with total cells and phage data, the no-seed-bank-L2 line shows deviation.** 
...
It is hard to tell from the plot, but surely phage has a very large effect.
...
In the non-infected lines we see that the populations differ between no-seed-bank lines and each of the with seed-bank treatments. But there is no difference between long and short seed banks. The trend is that the absence of a seed.bank results in higher cell densities. This is likely reflecting the "missing" spore population in this data.  

We also saw an interaction with time. We next compare the seed-bank treatments across time, separating the phage treatments, and focusing on the non-infected samples.
...
The "absence of seed bank" effect on population density is seen in a few of the samples from the later days of the experiment. We will need to look if this is complementary to an increase in spore density in the later part of the experiment in the absence of phage.

# Summary

The seed-bank treatment has a small effect on vegetative population dynamics, and only in the non-infected lines towards the end of the experiment. Phage infection results in no such difference observed.  
