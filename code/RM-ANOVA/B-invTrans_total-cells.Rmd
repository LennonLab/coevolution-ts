...
emm_sb.p <-
  ...
  emmeans(pairwise ~  phage) 

regrid(emm_sb.p$emmeans)

contrast(regrid(emm_sb.p$emmeans), method = "pairwise")

plot(regrid(emm_sb.p$emmeans))+
  theme_cowplot()+
  panel_border()+
  scale_x_log10()
...
p1 <- 
  ...
  mutate(contrast=fct_relevel(contrast, "long - short", after = 0))%>%
  #plot
  ggplot(aes(x=day, y=estimate, color=contrast))+
    #add 0 line
...
p2 <- coevo.pairs%>%
  ...
  mutate(contrast=fct_relevel(contrast, "long - short", after = 0))%>%
  #plot
  ggplot(aes(x=day, y=adj.p.value, color=contrast))+
    #add 0.05 signifcance thrshold
...
Post hoc estimates amd p-value for phage infected days 5-12 
comparing no seed bank to with seed bank

```{r}
 my.rg <-    stats::update(ref_grid(m.best), tran = make.tran("boxcox", param =  bx.cx$roundlam)) %>% 
  regrid() 

p.rg <- pairs(regrid(my.rg), simple=c("seed.bank"))%>%
  tidy()%>%
...
focal.pairs <- my.rg %>% 
  ...
  filter(phage == "SPO1") %>%
  group_by(time.fct)%>%
  mutate(density.ratio = estimate/estimate[seed.bank=="none"])%>%
  filter (Time == 5)%>%
  print()
 ...
 my.rg %>% 
  ...
  mutate(day=as.numeric(time.fct))%>%
  #plot
  ggplot(aes(x=day, y=estimate, color=seed.bank))+
    #add 0 line
  geom_hline(yintercept = 0, color="red")+
