...
cell.rm <- 
  ...
  arrange(seed.bank)%>%
  as.data.frame()%>%
  lme(trend ~ phage * seed.bank*Time, random = ~ 1 | line, 
...
Compare raw and observed data
```{r}
tmp.obs<- 
  d.decomp%>%
  # looking only at total cells
  filter(pop=="cell.ml")%>%
  select(Time,phage, seed.bank,line, observed, trend)

tmp.raw <- 
  d%>%
  filter(Time>=0)%>%
  select(Time,phage, seed.bank,line,raw=cell.ml)

tmp.both <- 
full_join(tmp.raw,tmp.obs,by = c("Time", "phage", "seed.bank", "line"))%>%
  mutate(dif=raw-observed)%>%
  arrange(Time)
  as.data.frame()
...
```{r}
lme(raw ~ phage * seed.bank*Time, random = ~ 1 | line, 
            correlation = corAR1(form = ~ 1 | line),
            data = tmp.both)%>%
  anova(.)

lme(observed ~ phage * seed.bank*Time, random = ~ 1 | line, 
            correlation = corAR1(form = ~ 1 | line),
            data = tmp.both)%>%
  anova(.)
...
lme(trend ~ phage * seed.bank*Time, random = ~ 1 | line, 
            correlation = corAR1(form = ~ 1 | line),
            data = tmp.both, na.action = "na.omit")%>%
  anova(.)
