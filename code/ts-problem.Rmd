...
In my ubderstanding this model tests the response `log.abd` (abundance data) to 2 fixed effects:`lim` treatment (limiting nutrient) and `day.fac` (Time as a factor). In addition wach of the chemostats (`cID`) is specified as a random effect. The auto-correlation is apecified by an `corAR1` being "*an autocorrelation structure of order 1*".
