...
broom.mixed::augment(rm.ar1)%>%
  ...
  geom_hline(yintercept = 0)+
  geom_point(aes(color=line), position = position_dodge(width = .5))+
  facet_wrap(~phage)+
  theme_cowplot()+panel_border()

broom.mixed::augment(rm.ar1)%>%
ggpubr::ggdensity(., x = ".resid", color = "line") +
  scale_x_continuous(limits = c(-2e8, 2e8)) +
  ggpubr::stat_overlay_normal_density(aes(color=line), linetype = "dashed")+
  facet_grid(seed.bank~phage)
```



# using the corAR1 model  - log transformed data
...
anova.lme(rm.ar1.log)
```

```{r}
broom.mixed::augment(rm.ar1.log)%>%
  ggplot(aes(Time,.resid)) +
  geom_hline(yintercept = 0)+
  geom_line(aes(color=line))+
  facet_grid(seed.bank~phage)+
...
```{r}
tmp <- broom.mixed::augment(rm.ar1)
...
tmp%>%
  ...
  filter(line=="L1")%>%
  pull(cell.ml)%>%
Box.test(., fitdf = 0, type="Lj")

ggpubr::ggdensity(tmp, x = ".resid", fill = "lightgray", title = "residuals") +
...
d.4stat%>%
  ...
  group_by(transfer,phage,seed.bank, line)%>%
  summarise(m=mean(cell.ml), v=sd(cell.ml))%>%
  ggplot(aes(x=line, y=m))+
  geom_pointrange(aes(ymin=m-v, ymax=m+v, color=transfer), position=position_dodge(width = .5))+
  facet_grid(seed.bank~phage)
```
...
d.4stat%>%
  mutate(transfer=if_else(as.logical(Time%%2), "mid", "end"))%>%
  group_by(transfer,phage,seed.bank,line)%>%
  mutate(t.cells =cell.ml/sd(cell.ml))%>%
  ungroup()%>%
  ggplot(aes(x=Time, y=t.cells))+
      geom_line(aes(group=flask,color=seed.bank), size=1, alpha=0.7)+
      facet_wrap(~phage)+
      theme_bw()+
      panel_border()+
...
```{r}
car::leveneTest(cell.ml ~ seed.bank, data = d.dif)
```

```{r}
d.4stat%>%
  # filter(phage=="SPO1")%>%
  # group_by(seed.bank,flask)%>%
  # summarise(n=n())
  friedman.test(cell.ml ~ Time|flask, data = . )
