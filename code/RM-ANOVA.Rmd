...
```{r}
anova(cell.rm.ARMA.p2q1,cell.rm.ARMA.p6q1,cell.rm.ARMA.p2q2,cell.rm.ARMA.p6q2,cell.rm.AR1)
```
...
plot_TukeyHSD(tuk)+ggtitle("With Phage")

tuk <- TukeyHSD(aov(cell.ml~seed.bank,data = filter(d, phage!="SPO1")))

plot_TukeyHSD(tuk)+ggtitle("No Phage")
...
There are 4 models with lowest AIC, those having (p=3 ,q=2 ); (p=4 ,q=2 );(p=5 ,q=1 );(p=6 ,q=1 )
or p=6, and withe q=1 or q=2. Over all the q=1 models seem to be doin better.
...
Is the improvment significant?  
```{r}
anova(phage.rm.ARMA.p4q2,phage.rm.ARMA.p5q1,phage.rm.ARMA.p3q2,phage.rm.ARMA.p6q1,phage.rm.AR1)
```
